...
const DB_PATH = path.join(app?.getPath?.('userData') ?? '.', 'interview-cache.db')
...
export class AnswerCache {
  ...
  private seedCommonQuestions() {
    const count = (this.db.prepare('SELECT COUNT(*) as n FROM answers').get() as any).n
  ...
  get(questionText: string, questionType: string): string | null {
    const hash = this.hashQuestion(questionText)
    const row = this.db
      .prepare('SELECT answer_text FROM answers WHERE hash = ? AND question_type = ?')
      .get(hash, questionType) as any
    return row?.answer_text ?? null
  }

  set(questionText: string, questionType: string, answerText: string) {
    const hash = this.hashQuestion(questionText)
    this.db
      .prepare(
